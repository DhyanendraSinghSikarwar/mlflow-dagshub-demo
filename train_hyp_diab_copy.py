...
with mlflow.start_run():
    ...
    # Create signature and log the model with input example
    signature = mlflow.models.infer_signature(X_train, grid_search.best_estimator_.predict(X_train))
    input_example = X_train.iloc[:5]  # Use first 5 rows as input example
    
    mlflow.sklearn.log_model(
        sk_model=grid_search.best_estimator_, 
        artifact_path="random_forest_model",  # Using artifact_path instead of deprecated name
        signature=signature,
